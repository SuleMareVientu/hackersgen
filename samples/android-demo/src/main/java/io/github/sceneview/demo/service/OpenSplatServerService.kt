...
object OpenSplatServerService {
    ...
    // Configured with generous timeouts for local Wi-Fi / LAN multi-hundred-MB transfers
    ...
    suspend fun uploadVideo(
        ...
        runCatching {
            ...
            val requestBody = createStreamingRequestBody(contentResolver, videoUri, fileSize, mediaType, onProgress)
            ...
            client.newCall(request).execute().use { response ->
                ...
                json.decodeFromString<UploadResponse>(bodyStr)
            }
        }
    }
    ...
    suspend fun uploadDataset(
        ...
        runCatching {
            val rootUrl = normalizeUrl(baseUrl)
            val uploadUrl = "$rootUrl/api/upload/dataset"
            val contentResolver = context.contentResolver
            val fileSize = contentResolver.openAssetFileDescriptor(datasetUri, "r")?.use { it.length } ?: -1L
            val mediaType = "application/zip".toMediaType()

            val requestBody = createStreamingRequestBody(contentResolver, datasetUri, fileSize, mediaType, onProgress)
            val multipartBody = MultipartBody.Builder()
                .setType(MultipartBody.FORM)
                .addFormDataPart("file", fileName, requestBody)
                .build()
    ...
    suspend fun clearJobLogs(baseUrl: String, jobId: String): Result<SimpleStatusResponse> = withContext(Dispatchers.IO) {
        runCatching {
            val rootUrl = normalizeUrl(baseUrl)
            val request = Request.Builder()
                .url("$rootUrl/api/jobs/$jobId/logs")
                .delete()
                .build()
